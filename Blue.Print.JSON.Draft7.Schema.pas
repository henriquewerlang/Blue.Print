unit Blue.Print.JSON.Draft7.Schema;
...
type
  ...
  // Forward type alias
  schemaArray = TArray<Blue.Print.JSON.Draft7.Schema.TSchema>;
  ...
  public type
    ...
    private
      ...
      FAdditionalItems: TSchema;
      FItems: TSchema.TObject.TItems;
      ...
      function GetMinLength: NonNegativeIntegerDefault0;
      function GetAdditionalItems: TSchema;
      function GetItems: TSchema.TObject.TItems;
      function GetMinItems: NonNegativeIntegerDefault0;
      function GetContains: TSchema;
      function GetMinProperties: NonNegativeIntegerDefault0;
    ...
    published
      ...
      property minLength: NonNegativeIntegerDefault0 read GetMinLength write FMinLength stored GetMinLengthStored;
      property pattern: System.String read FPattern write FPattern stored GetPatternStored;
      property additionalItems: TSchema read GetAdditionalItems write FAdditionalItems stored GetAdditionalItemsStored;
      property items: TSchema.TObject.TItems read GetItems write FItems stored GetItemsStored;
      property maxItems: nonNegativeInteger read FMaxItems write FMaxItems stored GetMaxItemsStored;
      property minItems: NonNegativeIntegerDefault0 read GetMinItems write FMinItems stored GetMinItemsStored;
      ...
      property minProperties: NonNegativeIntegerDefault0 read GetMinProperties write FMinProperties stored GetMinPropertiesStored;
...
procedure NonNegativeIntegerDefault0.SetAnonymous(const Value: any);
begin
  FAnonymous := Value;
...
function TSchema.GetObject: Blue.Print.JSON.Draft7.Schema.TSchema.TObject;
begin
  if not Assigned(FObject) then
    FObject := Blue.Print.JSON.Draft7.Schema.TSchema.TObject.Create;
...
function TSchema.TObject.GetMinLength: Blue.Print.JSON.Draft7.Schema.NonNegativeIntegerDefault0;
begin
  if not Assigned(FMinLength) then
    FMinLength := Blue.Print.JSON.Draft7.Schema.NonNegativeIntegerDefault0.Create;
...
function TSchema.TObject.GetAdditionalItems: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  if not Assigned(FAdditionalItems) then
    FAdditionalItems := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.GetItems: Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TItems;
begin
  if not Assigned(FItems) then
    FItems := Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TItems.Create;
...
function TSchema.TObject.GetMinItems: Blue.Print.JSON.Draft7.Schema.NonNegativeIntegerDefault0;
begin
  if not Assigned(FMinItems) then
    FMinItems := Blue.Print.JSON.Draft7.Schema.NonNegativeIntegerDefault0.Create;
...
function TSchema.TObject.GetContains: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  if not Assigned(FContains) then
    FContains := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.GetMinProperties: Blue.Print.JSON.Draft7.Schema.NonNegativeIntegerDefault0;
begin
  if not Assigned(FMinProperties) then
    FMinProperties := Blue.Print.JSON.Draft7.Schema.NonNegativeIntegerDefault0.Create;
...
function TSchema.TObject.GetAdditionalProperties: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  if not Assigned(FAdditionalProperties) then
    FAdditionalProperties := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.GetDefinitions: Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TDefinitions;
begin
  if not Assigned(FDefinitions) then
    FDefinitions := Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TDefinitions.Create;
...
function TSchema.TObject.GetProperties: Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TProperties;
begin
  if not Assigned(FProperties) then
    FProperties := Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TProperties.Create;
...
function TSchema.TObject.GetPatternProperties: Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TPatternProperties;
begin
  if not Assigned(FPatternProperties) then
    FPatternProperties := Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TPatternProperties.Create;
...
function TSchema.TObject.GetDependencies: Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TDependencies;
begin
  if not Assigned(FDependencies) then
    FDependencies := Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TDependencies.Create;
...
function TSchema.TObject.GetPropertyNames: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  if not Assigned(FPropertyNames) then
    FPropertyNames := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.GetType: Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TType;
begin
  if not Assigned(FType) then
    FType := Blue.Print.JSON.Draft7.Schema.TSchema.TObject.TType.Create;
...
function TSchema.TObject.GetIf: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  if not Assigned(FIf) then
    FIf := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.GetThen: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  if not Assigned(FThen) then
    FThen := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.GetElse: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  if not Assigned(FElse) then
    FElse := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.AddAllOf: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  Result := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.AddAnyOf: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  Result := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.AddOneOf: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  Result := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.GetNot: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  if not Assigned(FNot) then
    FNot := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.TItems.GetSchema: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  if not Assigned(FSchema) then
    FSchema := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.TItems.AddSchemaArray: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  Result := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
...
function TSchema.TObject.TDefinitions.GetSchema: TDynamicProperty<TSchema>;
begin
  if not Assigned(FSchema) then
    FSchema := TDynamicProperty<Blue.Print.JSON.Draft7.Schema.TSchema>.Create;
...
function TSchema.TObject.TProperties.GetSchema: TDynamicProperty<TSchema>;
begin
  if not Assigned(FSchema) then
    FSchema := TDynamicProperty<Blue.Print.JSON.Draft7.Schema.TSchema>.Create;
...
function TSchema.TObject.TPatternProperties.GetSchema: TDynamicProperty<TSchema>;
begin
  if not Assigned(FSchema) then
    FSchema := TDynamicProperty<Blue.Print.JSON.Draft7.Schema.TSchema>.Create;
...
function TSchema.TObject.TDependencies.GetSchema: Blue.Print.JSON.Draft7.Schema.TSchema;
begin
  if not Assigned(FSchema) then
    FSchema := Blue.Print.JSON.Draft7.Schema.TSchema.Create;
